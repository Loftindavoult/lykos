...
export async function updateContact(accountId, formData) {
  await requireStaff();
  const opt = (name, max = 300) => {
  ...
  await db.account.update({
    where: { id: accountId },
    data: {
      contactName: opt("contactName", 120),
      email: opt("email"),
      phone: opt("phone", 40),
      website: opt("website"),
      socialUrl: opt("socialUrl"),
    },
  });
  revalidatePath("/crm");
}
...
// A human overriding the grade sticks — leadScoreAuto flips to false so a
// later industry edit or re-import doesn't silently clobber their judgment.
export async function updateLeadScore(accountId, formData) {
  await requireStaff();
  const score = String(formData.get("leadScore") || "") || null;
  if (score && !["A", "B", "C", "D"].includes(score)) throw new Error("Invalid grade.");
  await db.account.update({ where: { id: accountId }, data: { leadScore: score, leadScoreAuto: false } });
  revalidatePath("/crm");
}
