...
export async function updateMrr(accountId, formData) {
  ...
  if (isCustom) {
    ...
    await db.account.update({
      where: { id: accountId },
      data: { mrr, mrrCustom: true, packageLevel: null, userCount: null, addonInventory: false, addonConsulting: false },
    });
  } else {
    ...
    await db.account.update({
      where: { id: accountId },
      data: { mrr, mrrCustom: false, packageLevel, userCount, addonInventory, addonConsulting },
    });
  }
  revalidatePath("/crm");
}
